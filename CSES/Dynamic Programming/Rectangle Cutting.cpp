    ...
    int dp(int rows , int cols){
        int & ret = memo[rows][cols] ;
        if(~ret) re ret;
        if(!rows ||!cols) re 0 ;
        if(rows == cols) re 0 ;
        if(rows == 1) re ret = cols - 1;
        if(cols == 1) re ret = rows - 1;
        ret = INT32_MAX ;
        for (int i = 1; i <cols ; ++i) {
            ret = min (ret , 1 + dp(rows , cols-i)+dp(rows, i)) ;
        }
        for (int i = 1; i <rows ; ++i) {
            ret = min (ret ,1 +  dp(rows-i , cols)+dp(i, cols)) ;
        }
        re ret ;
    }
    void solve(){
