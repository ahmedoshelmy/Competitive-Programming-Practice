...
vi a; int n ; int big_sum; vvi memo;
int  dp (int i, int s = 0 ){
    if(i == n) {
        re abs(big_sum - 2*s);
    }
    int &ret = memo[i][s];
    if(ret != -1) re ret;
    if(s+a[i] > (big_sum-(s+a[i]))){
        re dp(i+1,s);
    }
    int choice1 = dp(i+1,s);
    int choice2 = dp(i+1,s+a[i]);
...
void solve () {
    cin>>n;
    memo = vvi(n,vi(N,-1));
    big_sum = 0 ;a = vi (n); cin(a)
    for (int i = 0; i < n; ++i)big_sum+=a[i];
    int ans = dp(0,0);
    cout<<ans<<el;
